...
class Boink {
    ...
    do {
      try {
        ...
        if (error.status < 500 && error.status >= 400 && error.status != 429) {
          // this.log(`Invalid request for ${url}, maybe have new update from server | contact: https://t.me/ to get new update!`, "error");
          return { success: false, status: error.status, error: error.response.data.error || error.response.data.message || error.message };
        }
        this.log(`Request failed: ${url} | ${error.response.data ? JSON.stringify(error.response.data) : error.message} | trying again...`, "warning");
        success = false;
    ...
    try {
      ...
      if (response.success) {
        ...
        if (nextBoosterTime) {
          ...
          this.log(`Can't determine when to buy next boosts.`, "warning");
        }
      ...
      this.log(`Error sending purchase request boosts: ${error.message}`, "error");
      return { success: false, error: error.message };
    }
  }
    ...
    while (remainingSpins > 0) {
      ...
      try {
        ...
        if (response.success) {
          const result = response.data;
          this.log(
            `Spin success (${result.outcome}): Coin: ${result.newSoftCurrencyAmount.toString().white}${` | Shit: `.magenta}${result.newCryptoCurrencyAmount.toFixed(2).white}`.magenta,
            "custom"
          );
      ...
      for (const action of rewardedActions) {
        ...
        if (nameId === "SeveralHourlsRewardedAdTask" || nameId === "SeveralHourlsRewardedAdTask2") {
          ...
          try {
            ...
            this.log(`Do the task ${nameId.yellow} | status: ${`pending`.yellow}`);
          } catch (clickError) {
          ...
          if (action.secondsToAllowClaim > 0) {
            this.log(`Wait ${action.secondsToAllowClaim} second...`);
          ...
          try {
            ...
            if (claimResponse.success && claimResponse.data) {
              ...
              this.log(`Complete the mission ${nameId} success | reward: ${reward || JSON.stringify(result)}`, "success");
            } else {
              this.log(`Cannot claim reward for ${nameId} | ${JSON.stringify(claimResponse)}`, "error");
            }
          } catch (claimError) {
            this.log(`Error while claiming reward ${nameId}: Waiting time is still available!`, "warning");
          }
        }
  ...
  async handleAdTask(nameId, providerId) {
    try {
      ...
      await new Promise((resolve) => setTimeout(resolve, 2000));
      this.log(`Submit a request for a reward for an advertising task ${nameId}...`, "info");
      const claimResponse = await this.claimTask(nameId);

      if (claimResponse.success) {
        const result = claimResponse.data;
        const reward = result.prizeGotten;
        this.log(`Successfully completed ${nameId} advertising task | Reward: ${reward}`, "success");
      } else {
        this.log(`Unable to claim reward for promotion task ${nameId}. Status code: ${claimResponse.status}`, "error");
      }
    } catch (error) {
      this.log(`Error processing advertising task ${nameId}: waiting time still available!`, "error");
    }
  }
  ...
  async runAccount() {
    ...
    console.log(`=========Account ${this.accountIndex + 1}| ${firstName + " " + lastName} | ${settings.USE_PROXY ? this.proxyIP : "No proxy"} | Start later ${timesleep} second...`.green);
    ...
    try {
      ...
      if (userInfoResult.success) {
        ...
        for (const type of ["slotMachine"]) {
          ...
          if (spins > 0) {
            this.log(`Start spinning ${type} with ${spins} turn`, "yellow");
        ...
        return this.log(`Unable to get user information!  ${JSON.stringify(userInfoResult)}`, "error");
      }
    } catch (error) {
...
if (!settings.USE_PROXY) {
  console.log(colors.yellow("You are running the bot without proxies. Enable it by setting USE_PROXY in the .env file."));
}
